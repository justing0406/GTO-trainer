(() => {
  ...
  window.fetch = async (input, init) => {
    ...
    try {
      ...
      // Fall through to the original request if URL parsing is unavailable.
    }
  ...
  function updateVersionLabels() {
    ...
    if (advanced && !notice) {
      ...
      notice.innerHTML = `<strong>Advanced v2</strong><span>Mixed strategies are normal. A hand may correctly raise 70% and call 30%; feedback will show the approximate mix and explain why.</span>`;
      const toolbar = document.querySelector('.trainer-toolbar');
